...
export function LatencyTrendChart({ events }: LatencyTrendChartProps) {
  const [data, setData] = useState<number[]>(Array(30).fill(0));

  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      const newBinTotals = Array(30).fill(0);
      ...
      for (const event of events) {
        ...
        if (diffSeconds >= 0 && diffSeconds < 30) {
          ...
          newBinTotals[idx] += event.latencyMs;
          newBinCounts[idx]++;
        }
      }

      const newBinAverages = newBinTotals.map((tot, idx) => {
        const count = newBinCounts[idx];
        return count > 0 ? tot / count : 0;
      });

      setData(newBinAverages);
    }, 1000);
  ...
  const maxVal = useMemo(() => {
    ...
    return Math.ceil(max / 100) * 100; // Round up to nearest 100ms for clean grid
  }, [data]);
  ...
  // Dynamically determine chart color based on the current average latency
  const currentAvg = Math.round(data[29]);
  const chartColor = useMemo(() => {
    if (currentAvg === 0) return "var(--text-primary)";
    if (currentAvg < 100) return "var(--status-success)";
    if (currentAvg < 300) return "var(--status-warning)";
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <span style={{ fontSize: "20px", fontWeight: 700, fontFamily: "var(--font-mono)", color: chartColor }}>
            {currentAvg}ms
          </span>
          <span style={{ fontSize: "12px", color: "var(--text-muted)", marginLeft: "4px" }}>avg current</span>
        </div>
      </div>
      ...
      <div style={{ position: "relative", width: "100%", height: `${height}px` }}>
        <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
          ...
          {/* Area under the line */}
          <path d={areaD} fill="url(#latencyAreaGradient)" />

          {/* Main line path */}
          <path d={pathD} fill="none" stroke={chartColor} strokeWidth="1.75" strokeLinecap="round" />
